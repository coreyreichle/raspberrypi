...
your Raspberry Pi into a Wifi access point (SSID Pi_AP, Passphrase Raspberry),
that tunnels all traffic over the TOR network.
...
apt-get -y install wget hostapd isc-dhcp-server tor &>/dev/null
...
read -p "Write this address down, and hit [ENTER] to continue..."
